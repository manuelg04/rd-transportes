...
const services: Service[] = [
  {
    title: "Carga Seca",
    description:
      "Transporte seguro y eficiente de carga seca en todo el territorio nacional.",
    icon: Truck,
  ...
  },
  {
    ...
    description:
      "Especialistas en transporte de carga refrigerada, manteniendo la cadena de frío.",
    icon: Snowflake,
  ...
  },
  {
    ...
    description:
      "Servicios de Inspección, Transporte y Registro para una logística integral.",
    icon: FileCheck,
    gradient: "from-yellow-400 to-orange-500",
    image: "/contenedores.png",
  },
  {
    title: "Transporte de Ganado",
    description:
      "Transporte especializado de ganado en pie, garantizando el bienestar animal.",
    icon: Dog,
    gradient: "from-orange-500 to-yellow-400",
...
export function Services() {
  ...
  const prevImage = () => {
    ...
    if (currentService.images && currentService.images.length > 0) {
      setActiveImageIndex(
        (prevIndex) =>
          (prevIndex - 1 + currentService.images!.length) %
      ...
      >
        <ChevronRight className="h-4 w-4 text-gray-600" />
            ...
            <Card className="w-full lg:w-1/3 bg-white bg-opacity-90 shadow-xl">
              <div
                className={`h-2 bg-gradient-to-r ${currentService.gradient}`}
              ></div>
              <CardContent className="p-6 md:p-8">
                <div
                  className={`w-16 h-16 md:w-20 md:h-20 mx-auto rounded-full bg-gradient-to-br ${currentService.gradient} flex items-center justify-center mb-4 md:mb-6`}
                >
                  {React.createElement(currentService.icon, {
                    className: "w-8 h-8 md:w-10 md:h-10 text-white",
                  })}
                </div>
                <h3 className="text-2xl md:text-3xl font-semibold text-center text-gray-800 mb-3 md:mb-4">
